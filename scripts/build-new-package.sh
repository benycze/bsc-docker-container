...
(cd reps/bsc; make -j $JOBS GHCJOBS=$JOBS  PREFIX=$BSC_BUILD_ROOT all)
...
(cd reps/bsc-contrib; make -j $JOBS PREFIX=$BSC_CONTRIB_BUILD_ROOT)
...
(cd reps/bdw; make PREFIX=$BDW_FOLDER_BUILD_ROOT)
